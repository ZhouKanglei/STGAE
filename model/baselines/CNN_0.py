...
from config.config import CNN_WINDOW_STRIDE

class cnn(Model):
    """Learning Motion Manifolds with Convolutional Auto-encoders"""

    def __init__(self,
                 hidden_filters=[108, 128, 256],
                 dropout=0):
        ...
        for i in range(len(self.hidden_filters)):
            self.encoder.append(Sequential([
                Conv1D(filters=self.hidden_filters[i],
                       kernel_size=CNN_WINDOW_STRIDE, use_bias=True,
                       padding='same'),
                BatchNormalization(),
                Activation('relu'),
                MaxPool1D(pool_size=2) if 2 % 2 == 0 else Lambda(lambda x: x),
                Dropout(dropout),
            ], name='Encoder_%s' % (i + 1)))
        ...
        for i in range(len(self.hidden_filters)):
            self.decoder.append(Sequential([
                ...
                BatchNormalization(),
                Activation('relu'),
                UpSampling1D(size=2) if 2 % 2 == 0 else Lambda(lambda x: x),
                Dropout(dropout),
