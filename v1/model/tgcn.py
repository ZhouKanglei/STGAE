...
from config.config import ATTENTION, STRATEGY
...
class tgcn(Model):
    ...
    def __init__(self,
                 ...
                 layer_no=1,
                 A=None):
        super(tgcn, self).__init__(dynamic=True)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        h = self.conv(x)
        h = self.reshape(h)
        y = tf.einsum('ntvkc, kvw->ntwc', h, tf.multiply(self.A, self.M))
